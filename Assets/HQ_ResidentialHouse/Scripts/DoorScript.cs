...
#if ENABLE_INPUT_SYSTEM
		public Key openButton = Key.E; //Button on the keyboard to open the door
#else
...
#if ENABLE_INPUT_SYSTEM
        if (!Keyboard.current[controls.openButton].wasPressedThisFrame || controls.openMethod != OpenStyle.BUTTON)
            return;
#else
